...
def get_time_attributes_seqs(attributes_file):
    ...
    for line in lines:
        ...
        attributes_seq.append(attributes)
    # print(attributes_seq)
    df = pd.DataFrame(np.array(attributes_seq))
    y = df[3].values.astype(np.float32)
    # print(y)
    time_scaler = preprocessing.MinMaxScaler()
    time_y = time_scaler.fit_transform(y.reshape(-1, 1)).astype(np.float32)
    df[3] = time_y
    attributes_seq = df.values.tolist()
    # print(time_y)
    yy = time_scaler.inverse_transform(np.array(time_y))
    # print(yy)
    return attributes_seq, attribute_size, time_scaler
