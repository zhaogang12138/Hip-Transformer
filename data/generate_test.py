...
def sample_test_data(data_name, test_num=10, sample_type='random'):
    """
    sample_type:
    ...
    for user, user_seq in user_items.items():
        ...
        while len(test_samples) < test_num:
            if sample_type == 'random':
                sample_ids = np.random.choice(all_item, test_num, replace=True)
...
sample_test_data('HospitalBilling')