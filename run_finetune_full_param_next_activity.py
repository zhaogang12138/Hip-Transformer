...
from utils import EarlyStopping, get_activity_seqs, get_attributes_seqs, get_time_attributes_seqs, check_path, set_seed
...
def main():
    ...
    eval_dataloader = DataLoader(eval_dataset, batch_size=args.batch_size, drop_last=True, shuffle=True)
    # eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size)

    test_dataset = HiDataset(args, activity_seq, attributes_seq, time_attributes_seq, data_type='test')
    test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size, drop_last=True, shuffle=True)
    ...
    early_stopping = EarlyStopping(args.checkpoint_path, patience=10, verbose=True, acc=True)
    if args.do_eval:
