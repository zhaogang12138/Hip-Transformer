...
from utils import get_user_seqs_long, get_item2attribute_json, check_path, set_seed, get_attributes_seq_long
...
def main():
    ...
    parser.add_argument('--data_name', default='traffic_fine', type=str)
    ...
    args.attributes_file = args.data_dir + args.data_name + '_attributes.txt'
    ...
    attributes_seq = get_attributes_seq_long(args.attributes_file)
    ...
    print(args)
    # attribute_path = './data/traffic_dataset.csv'
    # attributes_dataframe = pd.read_csv(attribute_path, header=0)
    # attributes = attributes_dataframe.iloc[:, 2:-1]
    ...
    for epoch in range(args.pre_epochs):
        ...
        pretrain_dataset = PretrainDataset(args, user_seq, long_sequence, attributes_seq)
        # pretrain_sampler = RandomSampler(pretrain_dataset)
        pretrain_dataloader = DataLoader(pretrain_dataset, batch_size=args.pre_batch_size)
        # pretrain_dataloader = DataLoader(pretrain_dataset, sampler=pretrain_sampler, batch_size=args.pre_batch_size)

        trainer.pretrain(epoch, pretrain_dataloader,  attributes_seq)
